...
function godown() {
  if (!runlol) {
    ...
    setTimeout(scroll => {
      window.scrollTo(
          {top : $("#projects").offset().top, left : 0, behavior : "smooth"});
      enableScrolling()
    }, 300);
  }
}
...
function disableScrolling() { disableBodyScroll(window); }

function enableScrolling() { enableBodyScroll(window); }
...
// slide down title
$('#main').animate({'margin-top' : '40vh'}, 300);

// ye i could have just gotten a random gradient from uigradients but some of
// them suck so i just handpicked a few good ones
let randomthemes = [
  [ "#ff8008", "#ffc837" ], [ "#4A00E0", "#8E2DE2" ], [ "#FF512F", "#F09819" ],
  [ "#FF5F6D", "#FFC371" ], [ "#FF416C", "#FF4B2B" ], [ "#ee0979", "#ff6a00" ],
  [ "#ec008c", "#fc6767" ], [ "#f857a6", "#ff5858" ], [ "#00c3ff", "#ffff1c" ],
  [ "#e1eec3", "#f05053" ], [ "#a8ff78", "#78ffd6" ], [ "#B3FFAB", "#12FFF7" ],
  [ "#AAFFA9", "#11FFBD" ], [ "#5433FF", "#20BDFF" ], [ "#00c6ff", "#0072ff" ],
  [ "#4e54c8", "#8f94fb" ], [ "#396afc", "#2948ff" ], [ "#673AB7", "#512DA8" ],
  [ "#4776E6", "#8E54E9" ], [ "#DA22FF", "#9733EE" ]
];
// images curated by me (yes i could get random ones but it wouldn't look as
// nice :))
let randimages =
    [
    ...
    function rand(items) { return items[~~(items.length * Math.random())]; } let
        randcolor = rand(randomthemes);
...
// removed - tbh the image changer looked too inconsistent on reload, just
// changing the gradient is fine
...
function fetchSpotify() {
  // grab spotify currently playing data and update description - there is
  // probably a way better way to do this than long polling but I couldn't find
  // one
  fetch("https://api.carcraftz.dev/spotifycp")
          ...
          if (json["is_playing"]) {
            ...
            if (songname != lastsongname) {
              ...
              document.getElementById("nowplaying").innerHTML =
                  `<i class="fab fa-spotify"></i>&nbsp` + songname + " | " +
                  artist +
                  `&nbsp&nbsp&nbsp<img id = "trackimg" src = "${cover}">`
